...
# Execute commands
echo $(green "(1/6) Create resource group \"$RESOURCE_GROUP_NAME\"")
if [ $(az group exists --name $RESOURCE_GROUP_NAME) = false ]; then
  az group create --name $RESOURCE_GROUP_NAME --location $LOCATION -o none
fi
echo $(green "(2/6) Create storage account \"$TF_STORAGE_ACCOUNT\"")
...
echo $(green "(3/6) Get storage account key")
...
echo $(green "(6/6) Create secrets in GitHub")
